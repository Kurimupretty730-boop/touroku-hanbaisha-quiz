...
function chooseParallel(){selectorCard('平行モード',`<div class="row"><select id="cSel" class="select"><option value="all">全章</option>${[1,2,4,3,5].map(c=>`<option value="${c}">第${c}章</option>`).join('')}</select></div><p class="small">例：第1章なら「R1の章内問1→R2の章内問1→…→R6の章内問1→R1の章内問2…」。</p>`);$('#startMode').onclick=()=>startParallel($('#cSel').value)}
function parallelIds(c){let ps=c==='all'?Q:Q.filter(q=>q.chapter==c),chs=c==='all'?[1,2,4,3,5]:[+c],ids=[];chs.forEach(ch=>{let max=ch===3?40:20;for(let cq=1;cq<=max;cq++)for(let y=1;y<=6;y++){let q=Q.find(x=>x.year===y&&x.chapter===ch&&x.chapterQuestion===cq);if(q)ids.push(q.id)}});return ids}
...
function splitDisplayQuestion(text){
  ...
  const isSameDigit=(ch)=>/[0-9０-９]/.test(ch||'');

  function positionsFor(labels){
    const all=[];
    for(let i=0;i<s.length;i++){
      if(s[i]!==labels[0]) continue;
      if(!boundaryPrev(s[i-1])) continue;
      if(isSameDigit(s[i+1])) continue;
      const pos=[i]; let cursor=i+1, ok=true;
      for(let n=1;n<4;n++){
        let found=-1;
        for(let j=cursor;j<s.length;j++){
          if(s[j]===labels[n] && boundaryPrev(s[j-1]) && !isSameDigit(s[j+1])){found=j;break}
        }
        if(found<0){ok=false;break}
        pos.push(found);cursor=found+1;
      }
      if(!ok) continue;
      // 5番が後ろにあれば5択、なければ4択として扱う。
      let fifth=-1;
      for(let j=cursor;j<s.length;j++){
        if(s[j]===labels[4] && boundaryPrev(s[j-1]) && !isSameDigit(s[j+1])){fifth=j;break}
      }
      if(fifth>=0) pos.push(fifth);
      // 本文の途中の番号列を誤認しないよう、選択肢ブロックは後半に限定。
      if(pos[0] < s.length*0.12) continue;
      all.push(pos);
    }
    return all;
  }
  ...
  for(const pos of seqs){
    ...
    for(let n=0;n<pos.length;n++){
      const start=pos[n]+1;
      const end=n<pos.length-1?pos[n+1]:s.length;
    ...
    // 組合せ表の列見出し（a b c / a b c d）を本文末尾から分離。
    let headers=null;
